...
export default {
  en: {
    ...
    diff2Body:
      'Not a chat window bolted onto notes. It reads your PDFs and EPUBs, writes and links Markdown pages, files new material where it belongs, and edits your files directly — always with your approval.',
    ...
    // 2026-08-17, which is what makes naming it here allowed at all; the
    // message house gates that. Capability only — like GitHub sync above it,
    // what it costs is the pricing section's business, not this list's.
    does8:
    ...
    // The second sentence is the message house's wording for pillar 3, reused
    // as written; the first puts it in the negative so it belongs in this list.
    dont8:
      'No new structure to learn. It adapts to your folders, not the other way round — there is no import, because there is no format to import into.',
  ...
  zh: {
    ...
    diff2Body:
      '不是硬塞进笔记的聊天框。它读你的 PDF 和 EPUB、撰写和链接 Markdown 页面、把新材料归到该去的地方，并且经你批准直接改你的文件。',
    ...
    dont8:
      '不用学一套新结构。是软件适应你的目录，不是你去适应软件——没有「导入」这一步，因为根本没有要导入进去的格式。',
