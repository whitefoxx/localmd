...
export default {
  en: {
    ...
    privacy: 'No account · No upload · No usage caps · Just a web page and your folder',
  ...
  zh: {
    headline: '你来写，AI 来连，一切都在你本地的文件夹里。',
    subline:
      '一个 agent 住进你的文件夹，一座 wiki 绕着你的原件生长。它就地读文件夹里已有的 PDF、EPUB 和笔记，也够得着你浏览器里开着的标签页，在旁边写出互相链接的 Markdown。每条引用点回原文那一段，每个改动都等你点头。',
    openFolder: '打开本地文件夹',
    demo: '先试示例知识库',
    ...
    privacy: '没有账号 · 不上传 · 没有用量上限 · 只有一个网页，加你的文件夹',
    frameCaption: '示例知识库 · 不用选文件夹 · 不用填 Key · 关掉标签页就没了',
    source: '开源',
    sourceTitle: '代码可以读，也可以自己跑一份。MIT，在 GitHub 上',
    whyChrome: '为什么只有 Chrome 和 Edge？',
    whyChromeBody:
      '让一个网页读写你选定的文件夹，靠的是 File System Access API，目前只有 Chromium 系浏览器有。所以 localmd 需要 Chrome 或 Edge；Firefox 和 Safari 根本打不开本地文件夹。示例知识库不需要这些，在哪儿都能跑。',
    unsupported:
