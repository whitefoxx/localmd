      ...
      <div class="w-[560px] max-w-[90vw] max-h-[70vh] rounded-lg border border-border bg-bg-1 flex flex-col">
        ...
        <div class="panel-scroll p-4 space-y-5 text-sm">
          <p class="text-xs leading-relaxed text-fg-3">
            {{ $t('health.introBefore') }} <code class="rounded bg-bg-2 px-1 font-mono text-fg-2">[[1:b14-3]]</code>
            {{ $t('health.introMiddle') }}
            <code class="rounded bg-bg-2 px-1 font-mono text-fg-2">[[wikilinks]]</code> {{ $t('health.introAfter') }}
          </p>

          <section>
            <h3 class="text-xs uppercase tracking-wide text-fg-3">
              {{ $t('health.brokenHeading', { n: index.health.brokenLinks.length }) }}
            </h3>
            <p class="mb-2 text-xs text-fg-3">
              {{ $t('health.brokenDesc') }}
            </p>
            <div v-if="!index.health.brokenLinks.length" class="text-fg-3">{{ $t('health.allClear') }}</div>
          ...
          <section>
            <h3 class="text-xs uppercase tracking-wide text-fg-3">
              {{ $t('health.orphansHeading', { n: index.health.orphans.length }) }}
            </h3>
            <p class="mb-2 text-xs text-fg-3">
              {{ $t('health.orphansDesc') }}
            </p>
            <div v-if="!index.health.orphans.length" class="text-fg-3">{{ $t('health.allClear') }}</div>
