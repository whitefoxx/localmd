...
summary: MEMORY.md is what the agent remembers between chats; it is only ever written when you ask, and it is a file you can read and edit.
---
...
(Memory is what the agent carries without being asked. It can also look
back at your earlier chats in this folder when you ask it to — see
`working-with-the-agent`.)

If it exists, the agent reads it at the start of every chat and honours
...
The agent will not add to your memory on its own, and it will not quietly
...
If it has grown, ask the agent to tidy it.
